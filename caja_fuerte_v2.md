...
**FUNCIONAMIENTO COMPLETO DEL LED**  
**ALARMA DE AGUA**  
**FUNCIONAMIENTO CORRECTO DEL SERVOMOTOR Y DEL MÓDULO BLUETOOTH**  
**CORRECCIONES MENORES EN EL TIMEOUT DEL BLUETOOTH**

### Fallos corregidos

Se solucionó un problema técnico causado por las limitaciones de la placa, ya que la librería `SoftwareSerial.h` y `Servo.h` utilizan el mismo timer interno (Timer1) causando interferencias. La mejor solución fue utilizar `HardwareSerial` a pesar de sus limitaciones y dejar el `Timer1` para `Servo.h`. No se pudo utilizar el `Timer2` ya que las alarmas lo utilizaban en `Tone.h`.
### Código:
```cpp
// HARDWARE SERIAL
